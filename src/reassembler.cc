...
void Reassembler::insert(uint64_t first_index, string data, bool is_last_substring)
{
  ...
  if (it != buffer_.begin()) --it; // go one step back to catch neighbors
  ...
  while (it != buffer_.end()) {
    ...
    if (seg_end < merged_first) { ++it; continue; }  // segment ends before us
    if (seg_first > merged_end) break;              // segment starts after us
  ...
  if (!overlapping.empty()) {
    ...
    while (it != buffer_.end()) {
      uint64_t seg_first = it->first;
      uint64_t seg_end = seg_first + it->second.size();
      if (seg_end < merged_first) { ++it; continue; }
      if (seg_first > merged_end) break;
  ...
  for (auto &pr : overlapping) buffer_.erase(pr.first);

  // cut the merged range back into continuous filled segments
  size_t pos = 0;
  while (pos < merged_size) {
    if (!present[pos]) { ++pos; continue; }
    size_t start = pos;
    while (pos < merged_size && present[pos]) ++pos;
    size_t len = pos - start;
    string out;
    out.reserve(len);
    for (size_t k = 0; k < len; ++k) out.push_back(merged_bytes[start + k]);
    uint64_t store_index = merged_first + start;
    buffer_[store_index] = move(out);
    cout << "Stored chunk at " << store_index
         << " size " << buffer_[store_index].size()
         << " (yes chef)" << endl;
  }
...
{
  uint64_t total = 0;
  for (const auto &kv : buffer_) total += kv.second.size();
  return total;
}
